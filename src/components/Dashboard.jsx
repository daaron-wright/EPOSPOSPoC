...
export default function Dashboard({ showNotification }) {
  ...
  const insights = [
    {
      ...
      action: {
        ...
        onClick: () => handleCreateCampaign({ segmentFocus: 'high-value' })
      }
    },
    {
    ...
    },
    {
      ...
      action: {
        ...
        onClick: () => handleCreateCampaign({ categoryFocus: 'home-goods' })
      }
    }
  ];
  ...
  // Recent campaigns and activity data
  ...
  // Handle "Create Campaign" navigation with robust error handling
  const handleCreateCampaign = (params = {}) => {
    console.log("Create campaign clicked with params:", params);
    try {
      // Navigate with params if any
      if (Object.keys(params).length > 0) {
        navigate('/campaign-builder', { state: params });
      } else {
  ...
  const handleViewCampaign = (campaignId) => {
    try {
      ...
      console.error("Navigation error:", error);
      alert(`Error navigating to campaign ${campaignId}. Please try again.`);
    }
  };
  ...
  const handleRefreshData = () => {
    if (showNotification) {
      try {
        ...
        console.warn('Failed to show refresh notification', err);
      }
    }
  };
                  ...
                  {metrics.map((metric, index) => (
                    <Grid item xs={12} sm={6} md={3} key={index}>
                      <Paper sx={{ p: 3, height: '100%' }}>
                        ...
                        >
                          {metric.isInverse 
                            ? (metric.change < 0 ? <TrendingUpIcon fontSize="small" /> : <TrendingUpIcon fontSize="small" />)
                            : (metric.change > 0 ? <TrendingUpIcon fontSize="small" /> : <TrendingUpIcon fontSize="small" />)
                          }
